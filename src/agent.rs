...
use crate::{config::CliArgs, dex_monitoring::DexData, order_book::{OrderBook, Side}};
...
impl ArbitrageAgent {
    ...
    fn check_for_arbitrage(&self) {
        ...
        if dex_price > mid_cex_price {
            ...
            if let Some(cex_buy_price) = order_book.calculate_average_filled_price(token_a_buffer, Side::Ask) {
                // very trivial, not counting pool fees or gas fees
                let dex_net_sell_price = dex_price;
                if dex_net_sell_price > cex_buy_price * (Decimal::one() + Decimal::from(self.config.min_gain_margin) / Decimal::from(1_000_000)) {
                    let units_of_token_b_to_sell = token_a_buffer * dex_net_sell_price;
                    info!("Potential arbitrage: Buy on CEX {} units on token A and sell on DEX {} units of token B", token_a_buffer, units_of_token_b_to_sell);
                    info!("dex_price: = {}, mid_cex_price = {}", dex_price, mid_cex_price);
                }
            }
        }
        if dex_price < mid_cex_price {
            // trivial, not counting CEX fees
            if let Some(cex_sell_price) = order_book.calculate_average_filled_price(token_a_buffer, Side::Bid) {
                // very trivial, not counting pool fees or gas fees
                let dex_gross_buy_price = dex_price;
                if cex_sell_price > dex_gross_buy_price * (Decimal::one() + Decimal::from(self.config.min_gain_margin) / Decimal::from(1_000_000)) {
                    let units_of_token_b_to_buy = token_a_buffer / dex_gross_buy_price;
                    info!("Potential arbitrage: Sell on CEX {} units on token A and buy on DEX {} units of token B", token_a_buffer, units_of_token_b_to_buy);
                    info!("dex_price: = {}, mid_cex_price = {}", dex_price, mid_cex_price);
                }
            }
        }
